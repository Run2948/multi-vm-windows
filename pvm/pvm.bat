...
if not exist %cd%\v%v% (
	...
	if "1" == "%new%" (
       ...
       if not exist %cd%\python-%v%.msi (	
		  cscript download.vbs https://cdn.npm.taobao.org/dist/python/%v%/python-%v%.msi %cd%\python-%v%.amd64.msi
		  @echo 下载完成，正在安装...
	   )
		msiexec /a %cd%\python-%v%.msi /quiet TARGETDIR="%cd%\v%v%"	
	)	
) else (
